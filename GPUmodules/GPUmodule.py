...
class GPU_ITEM:
    ...
    def read_gpu_pstates(self):
        ...
        if not os.path.isfile(self.card_path + "power_dpm_state"):
            print("Error: Looks like DPM is not enabled: %s doesn't exist" % (self.card_path + "power_dpm_state"),
                  file=sys.stderr)
            self.compatible = False
            return
            #sys.exit(-1)
    ...
    def read_gpu_state_data(self):
        ...
        try:
            ...
            if os.path.isfile(self.card_path + "pp_power_profile_mode"):
                ...
                print("Error: card file doesn't exist: %s" % (self.card_path + "pp_power_profile_mode"),
                      file=sys.stderr)
                self.compatible = False
